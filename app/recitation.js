...
async function loadRecitationTexts() {
  ...
  // 挂到全局，供 ui.js 的 buildSearchIndex 访问
...
function bindRecitationEvents() {
  ...
  // 语音背诵后的操作按钮
  $('btn-recite-again')?.addEventListener('click', () => {
    // 再背一遍：重置当前段落的背诵状态
    hide($('recite-score'));
  ...
  $('btn-recite-clear-score')?.addEventListener('click', () => {
    // 删除本段背诵记录：隐藏分数，重置到初始状态
    hide($('recite-score'));
    hide($('recite-answer-reveal'));
    $('btn-recite-record').classList.remove('recording');
    $('recite-record-label').textContent = '开始背诵';
    $('recite-score-val').textContent = '0';
    $('recite-score-grade').textContent = '—';
    $('recite-score-feedback').textContent = '';
    $('recite-recognized').textContent = '';
    toast('已清除本段背诵记录');
  });

  $('btn-recite-replay')?.addEventListener('click', () => {
    // 回放：朗读当前段落的正确原文
    const seg = R.currentText?.segments?.[R.currentSegIdx];
    if (seg) speakChinese(seg.text.replace(/\n/g, '，'), 0.8);
  });
...
function renderFullText(text) {
  ...
  let html = lines.map(line =>
    line.trim() ? `<div class="recite-line">${escapeHtml(line)}</div>` : ''
  ).join('');

  // 白话文翻译
  if (text.translation) {
    html += `<div class="recite-translation">
      <div class="ra-label">🈶 白话文翻译</div>
      <div class="rt-text">${escapeHtml(text.translation)}</div>
    </div>`;
  }
  ...
  if (text.word_notes?.length) {
    html += `<div class="recite-word-notes">
      <div class="ra-label">📖 词语注释</div>
      <div class="rwn-grid">
        ${text.word_notes.map(n => `
          <div class="rwn-item">
            <span class="rwn-word">${escapeHtml(n.word)}</span>
            <span class="rwn-note">${escapeHtml(n.note)}</span>
          </div>`).join('')}
      </div>
    </div>`;
  }
  ...
  if (text.annotation) {
    html += `<div class="recite-annotation">
      <div class="ra-label">📝 创作背景</div>
      <div class="ra-text">${escapeHtml(text.annotation)}</div>
    </div>`;
  }
  ...
  if (text.key_points?.length) {
    html += `<div class="recite-keypoints">
      <div class="ra-label">🔑 学习重点</div>
      ${text.key_points.map(p => `<div class="rk-item">· ${escapeHtml(p)}</div>`).join('')}
    </div>`;
  }
