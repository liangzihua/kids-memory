...
[![Version](https://img.shields.io/badge/版本-1.1.0-orange)](https://github.com/liangzihua/kids-memory)
...
### 📖 语文学习中心
语文所有功能集中在一个页面内嵌展示，无需跳转：

- **🏮 古诗文背诵**：小学33首 + 唐宋66首，含白话文翻译、逐字注释、创作背景，支持原文朗读、段落填空、语音背诵评分
- **🔡 拼音学习**：声母韵母表（21+36）、四声详解与标调口诀、多音字48个、看拼音写词语、看词语写拼音
- **📝 生字词语**：小学生字（含偏旁部首笔画）、1-6年级语文词语，支持朗读
- **📜 成语**：3000条常用成语，含释义和例句，支持搜索
- **✍️ 古诗题库**：75首必背古诗，填空练习+闪卡模式

### 🔤 英语学习中心
- **单词速记**：音节着色辅助拼读（ver|sion 红蓝分色），SM-2 间隔复习算法
...
- **导航**：历史栈路由（`_pageHistory`），统一返回上一页

### 跨模块通信
esbuild 打包后模块作用域隔离，通过 `window` 桥接：
```js
window._showPage   // 页面跳转（记录历史栈）
window._goBack     // 返回上一页
window._recitationTexts  // 古诗词数据
window._pinyinData       // 拼音数据
window._renderPinyinToElement  // 拼音渲染函数
```
...
- 麦克风：仅用于语音跟读和背诵评分，录音处理完即销毁
- 相机：仅用于拍照导入题目，照片解析后即销毁
- 网络：仅用于用户主动下载词库，不进行后台数据上报
...
### v1.1.0（2026-06-23）
- 新增语文学习中心（所有子功能内嵌，不跳页）
- 新增拼音学习模块（声母韵母/四声/多音字/练习）
- 主页语文快捷区（4个快捷按钮直接定位 Tab）
- 页面导航重构（历史栈，🏠返回主页按钮）
- 英文单词音节着色辅助拼读
- 主页 UI 优化（4张卡片横向铺满，彩色渐变 header）
- 修复华为审核隐私标签问题
