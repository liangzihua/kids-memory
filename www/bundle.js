(() => {
  ...
  function speak(text, lang = "en-US", rate = 0.8, onEnd, pitch = 1) {
  ...
  function speakChinese(text, rate = 0.9) {
    speak(text, "zh-CN", rate, void 0, 1.05);
  }
  ...
  async function refreshVocabStats() {
    ...
    if (deckSel) {
      deckSel.innerHTML = '<option value="all">\u5168\u90E8\u82F1\u8BED\u8BCD\u6C47</option>' + decks.filter((d) => d.subject === "english").map((d) => `<option value="${d.id}">${escapeHtml(d.name)}</option>`).join("");
    }
  }
  ...
  async function renderTwisterList() {
    ...
    E.phrasesData.tongue_twisters?.forEach((tt) => {
      ...
      <div class="twister-actions">
        <button class="btn-secondary twister-speak" data-text="${escapeHtml(tt.text)}">\u{1F50A} \u542C</button>
    ...
    filtered.forEach((g) => {
      ...
      <div class="grammar-examples">
        ${g.examples.map((e) => `<div class="grammar-ex">
          <button class="grammar-speak" data-text="${escapeHtml(e.split(".")[0].replace(/'/g, ""))}">\u{1F50A}</button>
          <span>${escapeHtml(e)}</span>
        </div>`).join("")}
      </div>
      ${g.tips.length ? `<div class="grammar-tips">
        ${g.tips.map((t) => `<div class="grammar-tip">\u{1F4A1} ${escapeHtml(t)}</div>`).join("")}
      </div>` : ""}
    `;
  ...
  function renderRecitationListPage(cat = "all", query = "") {
    ...
    if (!texts.length) {
      list.innerHTML = `<div style="text-align:center;padding:48px;color:var(--color-text-sub)">${query ? `\u672A\u627E\u5230"${escapeHtml2(query)}"` : "\u6682\u65E0\u5185\u5BB9"}</div>`;
      return;
    }
    const groups = {};
    texts.forEach((t) => {
    ...
    keys.forEach((grade) => {
      ...
      items.forEach((text) => {
        ...
        <div class="rti-info">
          <div class="rti-title">${escapeHtml2(text.title)}</div>
          <div class="rti-meta">${text.dynasty ? text.dynasty + " \xB7 " : ""}${escapeHtml2(text.author || "")}${text.category ? " \xB7 " + text.category : ""}</div>
        </div>
  ...
  function renderFullText(text) {
    ...
    el.innerHTML = lines.map((line) => `<div class="recite-line">${escapeHtml2(line)}</div>`).join("");
  }
  ...
  function renderRecitationList(container, gradeFilter) {
    ...
    filtered.forEach((text) => {
      ...
      <div class="recit-info">
        <div class="recit-title">${escapeHtml2(text.title)}</div>
...
\u786E\u5B9A\u8981\u4E0B\u8F7D\u5417\uFF1F`)) {
      ...
      document.querySelectorAll(".import-mode-btn").forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          if (isAppend && App.currentProfile) {
            ...
            if (sel) {
              sel.innerHTML = '<option value="">-- \u9009\u62E9\u8981\u8FFD\u52A0\u7684\u9898\u5E93 --</option>' + decks.map((d) => `<option value="${d.id}">${escapeHtml3(d.name)}\uFF08${App.allProfileCards.filter((c) => c.deckId === d.id).length}\u5F20\uFF09</option>`).join("");
            }
          }
        });
      });
  ...
  function renderProfilesGrid(profiles) {
    ...
    profiles.forEach((p) => {
      ...
      <div class="profile-name">${escapeHtml3(p.name)}</div>
  ...
  async function openSubjectDecksPage(subject) {
    ...
    if (!profile) return;
    App.allProfileCards = await CardManager.getByProfile(profile.id);
    const subjectNames = { chinese: "\u8BED\u6587", english: "\u82F1\u8BED", math: "\u6570\u5B66", custom: "\u81EA\u5B9A\u4E49" };
    const subjectFilter = (card) => {
      if (card.subject === subject) return true;
    ...
    $3("subject-decks-title").textContent = `${subjectNames[subject] || subject}\u9898\u5E93`;
    ...
    subjectDecks.forEach((deck) => {
      ...
      <div class="sdb-deck-header">
        <span class="sdb-deck-name">${escapeHtml3(deck.name)}</span>
  ...
  function renderCard() {
    ...
    $3("card-front-text").textContent = displayFront;
  ...
  function renderCharInfo(card) {
    ...
    if (card.type === "char" && (card.radical || card.strokes)) {
      const parts = [];
      if (card.radical) parts.push(`<span class="char-info-item"><span class="cii-label">\u90E8\u9996</span><span class="cii-val">${escapeHtml3(card.radical)}</span></span>`);
      ...
      if (card.structure) parts.push(`<span class="char-info-item"><span class="cii-label">\u7ED3\u6784</span><span class="cii-val">${escapeHtml3(card.structure)}</span></span>`);
  ...
  function showImportPreview(cards) {
    ...
    cards.slice(0, 20).forEach((c) => {
      ...
      item.innerHTML = `<span class="preview-front">${escapeHtml3(c.front)}</span><span class="preview-back">${escapeHtml3(c.back)}</span>`;
      list.appendChild(item);
    });
    if (cards.length > 20) {
  ...
  async function renderLibrary() {
    ...
    const subjectLabel_ = { chinese: "\u8BED\u6587", english: "\u82F1\u8BED", math: "\u6570\u5B66", custom: "\u81EA\u5B9A\u4E49" };
    ...
    subjectOrder.forEach((subj) => {
      ...
      subjDecks.forEach((deck) => {
        ...
        <div class="deck-info">
          <div class="deck-name">${escapeHtml3(deck.name)}</div>
        ...
        <div class="deck-actions">
          ...
          <button class="btn-deck-delete" data-deck="${deck.id}" data-name="${escapeHtml3(deck.name)}" title="\u5220\u9664\u9898\u5E93">\u{1F5D1}</button>
        </div>
      `;
...
\u5220\u9664\u540E\u8BE5\u9898\u5E93\u7684\u6240\u6709\u5361\u7247\u548C\u5B66\u4E60\u8BB0\u5F55\u5C06\u6E05\u7A7A\uFF0C\u65E0\u6CD5\u6062\u590D\u3002`)) return;
  ...
  async function searchCards(query) {
    ...
    if (!container) return;
    if (!results.length) {
      container.innerHTML = `<div class="search-empty">\u6CA1\u6709\u627E\u5230"${escapeHtml3(query)}"\u76F8\u5173\u5185\u5BB9</div>`;
      show3(container);
      return;
    }
    ...
    results.forEach((card) => {
      ...
      const hi = (text) => {
        if (!text) return "";
        const re = new RegExp(`(${escapeHtml3(query).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")})`, "gi");
        return escapeHtml3(text).replace(re, "<mark>$1</mark>");
      };
      ...
      <div class="sri-content">
        <div class="sri-front">${hi(card.front)}${card.phonetic ? `<span class="sri-phonetic"> ${escapeHtml3(card.phonetic)}</span>` : ""}</div>
  ...
  async function renderStats() {
    ...
    hard.forEach((c) => {
      ...
      item.innerHTML = `<span class="hard-card-front">${escapeHtml3(c.front)}</span><span class="hard-card-rate">\u6B63\u786E\u7387 ${rate}%</span>`;
      list.appendChild(item);
    });
  }
  ...
  async function renderSettings() {
    ...
    profiles.forEach((p) => {
      ...
      row.innerHTML = `<span style="font-size:24px">${p.avatar}</span><span style="flex:1">${escapeHtml3(p.name)}</span>`;
  ...
  function loadVoiceSelectors() {
    const populate = (selectId, lang) => {
      ...
      sel.innerHTML = '<option value="">\uFF08\u81EA\u52A8\u9009\u6700\u4F73\uFF09</option>' + voices.map((v) => {
        ...
        return `<option value="${escapeHtml3(v.name)}" ${v.name === savedName ? "selected" : ""}>${escapeHtml3(v.name)}${quality}${online}</option>`;
      }).join("");
...
\u6B64\u64CD\u4F5C\u5C06\u5220\u9664\u6240\u6709\u5361\u7247\u548C\u5B66\u4E60\u8BB0\u5F55\uFF0C\u65E0\u6CD5\u64A4\u9500\uFF01`)) return;
  ...
  async function openManageProfiles() {
    ...
    profiles.forEach((p) => {
      ...
      <div class="manage-profile-info">
        <div class="manage-profile-name">${escapeHtml3(p.name)}</div>
      ...
      <div class="manage-profile-btns">
        ...
        <button class="btn-delete-sm" data-id="${p.id}" data-name="${escapeHtml3(p.name)}">\u5220\u9664</button>
      </div>
    `;
  ...
  function subjectLabel(subject) {
    return { chinese: "\u8BED\u6587", english: "\u82F1\u8BED", math: "\u6570\u5B66", custom: "\u81EA\u5B9A\u4E49" }[subject] || "";
  }
    ...
    if (input && !input.dataset.bound) {
      ...
      input.addEventListener("input", () => {
        ...
        if (q) {
          ...
          $3("search-page-count").textContent = "";
        }
      });
  ...
  async function runSearch(query) {
    ...
    const hiText = (text) => {
      if (!text) return "";
      return escapeHtml3(text).replace(
        new RegExp(`(${escapeRegex(escapeHtml3(q))})`, "gi"),
        "<mark>$1</mark>"
      );
    };
    const groupResults = [];
    ...
    for (const { deck, cards } of Object.values(_searchIndex)) {
      if (_searchFilter !== "all" && deck.subject !== _searchFilter) continue;
      ...
      if (matched.length) {
        groupResults.push({ deck, matched: matched.slice(0, 20) });
        totalCount += matched.length;
      }
    }
    if (!groupResults.length) {
      container.innerHTML = `<div class="search-empty-full">\u6CA1\u6709\u627E\u5230 <strong>${escapeHtml3(query)}</strong> \u76F8\u5173\u5185\u5BB9<br><span style="font-size:var(--font-size-xs);color:var(--color-text-hint)">\u8BD5\u8BD5\u6362\u4E2A\u5173\u952E\u8BCD\uFF0C\u6216\u5148\u4E0B\u8F7D\u66F4\u591A\u8BCD\u5E93</span></div>`;
      show3(container);
      hide3(hint);
    ...
    container.innerHTML = "";
    groupResults.forEach(({ deck, matched }) => {
      const section = document.createElement("div");
      ...
      <div class="sq-deck-header">
        ...
        <span class="sq-deck-name">${escapeHtml3(deck.name)}</span>
    ...
    `;
      matched.forEach((card) => {
        ...
        <div class="sq-item-main">
          <div class="sq-front">${hiText(card.front)}${card.phonetic ? `<span class="sq-phonetic">${escapeHtml3(card.phonetic)}</span>` : ""}</div>
